...
bool encryptFile(const std::string& inFile,
    ...
    for (char ch : norm) {
        if (ch == ' ') continue;
        stepRotors(r1, r2, r3);
        cipher += mapForward(r1, mapForward(r2, mapForward(r3, ch)));
    }
    // Re-agrupar en 5
    std::string out;
...
bool decryptFile(const std::string& inFile,
    ...
    // Eliminar espais
    std::string packed;
    for (char ch : raw) if (ch != ' ' && std::isalpha(ch)) packed += ch;
    std::string plain;
    for (char ch : packed) {
        stepRotors(r1, r2, r3);
        plain += mapBackward(r3, mapBackward(r2, mapBackward(r1, ch)));
    }
    // No re-agrupar; mostrar texto continuo
